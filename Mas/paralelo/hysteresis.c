...
void non_max_supp(short *mag, short *gradx, short *grady, int nrows, int ncols, unsigned char *result)
{
        ...
        ****************************************************************************/
        ...
        for(
                ...
                // printf("ncols %d\n", ncols);
                #pragma omp parallel shared(magptr,gxptr,gyptr,resultptr) private(colcount,m00,z1,z2,mag1,mag2,gx,gy,xperp,yperp,magptr_copia,gxptr_copia,gyptr_copia,resultptr_copia,ncols) num_threads(cant_threads)
                {
                        // printf("\n---\n");
                        #pragma omp for
                        for (colcount=1;
