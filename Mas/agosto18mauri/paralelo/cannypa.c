...
int main(int argc, char *argv[])
{
        ...
        ****************************************************************************/
        sprintf(outfilename, "imagen-paralelo.pgm");
        // sprintf(outfilename, "%s_s_%3.2f_l_%3.2f_h_%3.2f-pa.pgm", infilename, sigma, tlow, thigh);
                     ...
                     short int **delta_x, short int **delta_y)
{
        ...
        ****************************************************************************/
        ...
        int cant_threads = 4;
        int i, j;
        inicioX = omp_get_wtime();
        // #pragma omp parallel shared(smoothedim,delta_x) num_threads(cant_threads)
        // {
        // #pragma omp for
        // for (i = 0; i < (rows*cols); i=i+cols) {
        //         for (j = 1; j < cols-1; j++) {
        //                 (*delta_x)[i+j] = smoothedim[i+j+1] - smoothedim[i+j-1];
        //                 // (*delta_y)[pos] = smoothedim[pos+cols] - smoothedim[pos-cols];
        //                 // (*delta_y)[i+cols] = smoothedim[i+cols+cols] - smoothedim[i+cols-cols];
        //         }
        // }
        contador = 0;
        for (i = cols+1; i < (rows*cols-cols); i=i+cols) {
                for (j = 1; j < cols-1; j++) {
                        (*delta_x)[i+j] = smoothedim[i+j+1] - smoothedim[i+j-1];
        ...
        //         (*delta_x)[i+cols] = smoothedim[i+cols] - smoothedim[i+cols-1];
        // }

        // //para la fila 0 y la ultima en X
        // contador = 0;
        ...
        // }
        //
        // //para la columna 0 y la ultima en Y
        ...
        printf("(derrivative_x_y) derX: %f segundos\n", (finX-inicioX));
        ...
        ****************************************************************************/
        ...
        inicioY = omp_get_wtime();
        // #pragma omp parallel for shared(pos,smoothedim)  num_threads(cant_threads)
        // for(c=0; c<cols; c++) {
        //         pos = c;
        ...
        // }
        finY = omp_get_wtime();
        printf("(derrivative_x_y) derY: %f segundos\n", (finY-inicioY));

        fin = omp_get_wtime();
        printf("(derrivative_x_y) %f segundos\n", (fin-inicio));
}
                     ...
                     short int **smoothedim)
{
        ...
        ****************************************************************************/
        ...
        double inicioX, finX, inicioY, finY, inicio, fin;
        inicioX = omp_get_wtime();
        inicio = omp_get_wtime();

        int cant_threads = 4;
        // #pragma omp parallel default(shared) private(r,c,cc,dot,sum) num_threads(cant_threads)

        /****************************************************************************
        * Blur in the x - direction.
        ****************************************************************************/
        if(VERBOSE) printf("   Bluring the image in the X-direction.\n");
        #pragma omp parallel for shared(image,tempim,smoothedim) private(r,c,cc,rr,dot,sum) num_threads(cant_threads)
        // {
        for(r=0; r<rows; r++) {
                // #pragma omp for
                for(c=0; c<cols; c++) {
        ...
        // }
        // finX = omp_get_wtime();
        // printf("TIEMPO PARALELO X %f segundos\n\n", (finX-inicioX));
        // end = clock();

        // #pragma omp parallel shared(tempim,smoothedim) private(r,c,rr,dot,sum,rows,cols) num_threads(cant_threads)
        // {
        /****************************************************************************
        ...
        ****************************************************************************/
        ...
        fin = omp_get_wtime();
        // printf("TIEMPO PARALELO X %f segundos\n\n", (finY-inicioY));
        printf("(gaussian_smooth) %f segundos\n", (fin-inicio));
        // printf("(gaussian_smooth) %f segundos\n", (fin-inicio));
        ...
        // printf("TIEMPO PARALELO COMUN %.4g segundos\n\n", (time_in_seconds));
}
