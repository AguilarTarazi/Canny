...
int main(int argc, char *argv[])
{
   ...
   ****************************************************************************/
   ...
   if(rank==0){
       ...
       ****************************************************************************/
       sprintf(outfilename, "im_mpi.pgm");
        ...
        short int **delta_x, short int **delta_y)
{
   ...
   ****************************************************************************/
   ...
   if(((*delta_x_temp) = (short *) calloc(rows*cols/numtasks, sizeof(short))) == NULL){
      fprintf(stderr, "Error allocating the delta_x image.\n");
      exit(1);
   }
   if(((*delta_y_temp) = (short *) calloc(rows*cols/numtasks, sizeof(short))) == NULL){
      fprintf(stderr, "Error allocating the delta_x image.\n");
      exit(1);
   }
   ...
   * losing pixels.
   ****************************************************************************/
   short int *smoothedim_temp;
   // smoothedim_temp = calloc(rows*cols/numtasks, sizeof(short));
   if( ((*smoothedim) = (short int *) calloc((rows*cols)/numtasks, sizeof(short int))) == NULL){
      fprintf(stderr, "Error allocating the delta_x image.\n");
      exit(1);
  }
  ...
  // MPI_Bcast(&, 1, MPI_INT, 0, MPI_COMM_WORLD);
   ...
   printf("ANTES SCATTER\n");
   MPI_Scatter (smoothedim,(rows*cols)/numtasks,MPI_SHORT,smoothedim_temp,(rows*cols)/numtasks,MPI_SHORT,0,MPI_COMM_WORLD);
   printf("DESPUES SCATTER\n");

       clock_t ini_derivate_x, fin_derivate_x;
       ini_derivate_x = clock();
       if(VERBOSE) printf("   Computing the X-direction derivative.\n");

       for (int i = cols; i < (rows*cols-cols)/numtasks; i=i+cols) {
    ...
    MPI_Gather(delta_x_temp,(rows*cols)/numtasks,MPI_SHORT,delta_x,(rows*cols)/numtasks,MPI_SHORT,0,MPI_COMM_WORLD);
    MPI_Gather(delta_y_temp,(rows*cols)/numtasks,MPI_SHORT,delta_y,(rows*cols)/numtasks,MPI_SHORT,0,MPI_COMM_WORLD);
   if(rank==0){
       for (int j = 1; j < cols-1; j++) {
        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
   ...
   if(VERBOSE) printf("   Bluring the image in the X-direction.\n");
    ...
    // }
    // float *recvbuf = (float *) calloc((rows*cols)/numtasks, sizeof(float));
    // float *tempim_aux = (float *) calloc((rows*cols)/numtasks, sizeof(float));
    // int cant_elem_por_proc = (rows*cols)/numtasks;
    // printf("ANTES DE SCATTER, rank %d\n", rank);
    // MPI_Scatter (image,(rows*cols)/numtasks,MPI_FLOAT,recvbuf,(rows*cols)/numtasks,MPI_FLOAT,0,MPI_COMM_WORLD);
    // printf("DESPUES DE SCATTER, rank %d\n", rank);
        for(r=0;r<rows;r++){
            for(c=0;c<cols;c++){
                dot = 0.0;
                sum = 0.0;
    ...
    //     printf("ANTES DEL GATHER, rank %d\n", rank);
    // MPI_Gather(tempim_aux,(rows*cols)/numtasks,MPI_FLOAT,tempim,(rows*cols)/numtasks,MPI_FLOAT,0,MPI_COMM_WORLD);
    // printf("DESPUES DEL GATHER, rank %d\n", rank);
    // if(rank==0){
