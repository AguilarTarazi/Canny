...
int main(int argc, char *argv[])
{
   ...
   ****************************************************************************/
   sprintf(outfilename, "imagen_openmp.pgm");
   ...
   printf("\nTiempo total del programa: \t\t%.10g segundos\n\n", fin-ini);
   return 0;
}
        ...
        short int **delta_x, short int **delta_y)
{
   ...
   ****************************************************************************/
   ...
   double ini_derivate_x = omp_get_wtime();
   #pragma omp parallel for shared(smoothedim,delta_x,delta_y) num_threads(cant_threads)
   for (int i = cols; i < (rows*cols-cols); i=i+cols) {
           for (int j = 1; j < cols-1; j++) {
                   (*delta_x)[i+j] = smoothedim[i+j+1] - smoothedim[i+j-1];
                   (*delta_y)[i+j] = smoothedim[i+j+cols] - smoothedim[i+j-cols];
           }
   }
   double fin_derivate_x = omp_get_wtime();
   ...
   printf("Tiempo primer bloque paralelo: \t\t%.10f segundos\n", fin_derivate_x - ini_derivate_x);

   double fin_derivate = omp_get_wtime();
   printf("Tiempo total funcion: \t\t\t%.10f segundos\n", fin_derivate - ini_derivate);
   printf("=============================================================\n");
}
        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
   double ini_blur_x, fin_blur_x;
   int cant_threads = 8;
   if(VERBOSE) printf("   Bluring the image in the X-direction.\n");
   ini_blur_x = omp_get_wtime();

   #pragma omp parallel for shared(image, tempim) private(r,c,cc,dot,sum) num_threads(cant_threads)
   for(r=0;r<rows;r++){
      for(c=0;c<cols;c++){
         dot = 0.0;
         sum = 0.0;
   ...
   fin_blur_x = omp_get_wtime();
   printf("Tiempo primer bloque paralelo: \t\t%.10f segundos\n", fin_blur_x - ini_blur_x);
   /****************************************************************************
   * Blur in the y - direction.
   ****************************************************************************/
   double ini_blur_y,fin_blur_y;
   if(VERBOSE) printf("   Bluring the image in the Y-direction.\n");
   ini_blur_y = omp_get_wtime();

   #pragma omp parallel for shared(tempim, smoothedim) private(r,c,rr,dot,sum) num_threads(cant_threads)
       for(c=0;c<cols;c++){
          for(r=0;r<rows;r++){
             sum = 0.0;
             dot = 0.0;
   ...
   fin_blur_y = omp_get_wtime();
   printf("Tiempo segundo bloque paralelo: \t%.10f segundos\n", fin_blur_y - ini_blur_y);
	...
	float tlow, float thigh, unsigned char *edge)
{
        ...
        ****************************************************************************/
        int cant_threads = 8;
        // Primero
        double ini_hyst_first_for,fin_hyst_first_for;
        ini_hyst_first_for = omp_get_wtime();
        #pragma omp parallel shared(nms,edge,pos) num_threads(cant_threads)
        {
            #pragma omp for
        ...
        fin_hyst_first_for = omp_get_wtime();
        printf("Tiempo primer bloque paralelo: \t\t%.10f segundos\n", fin_hyst_first_for - ini_hyst_first_for);
        ...
        ****************************************************************************/
        ...
        double ini_cuartofor,fin_cuartofor;
        ini_cuartofor = omp_get_wtime();

        // #pragma omp parallel shared(edge,hist,mag) num_threads(cant_threads)
        // {
        // //     // printf("%d\n",omp_get_thread_num());
        //     #pragma omp for
        ...
        fin_cuartofor = omp_get_wtime();
        printf("Tiempo segundo bloque paralelo: \t%.10f segundos\n", fin_cuartofor - ini_cuartofor);
        ...
        ****************************************************************************/
        double ini_hyst_third_for,fin_hyst_third_for;
        ini_hyst_third_for = omp_get_wtime();

        #pragma omp parallel shared(pos,edge,mag) num_threads(cant_threads)
        {
            #pragma omp for
            for(pos=0; pos<(cols*rows); pos++) {
        ...
        fin_hyst_third_for = omp_get_wtime();
        printf("Tiempo tercer bloque paralelo: \t\t%.10f segundos\n", fin_hyst_third_for - ini_hyst_third_for);

        /****************************************************************************
        * Set all the remaining possible edges to non-edges.
        ****************************************************************************/
        double ini_hyst_last_for,fin_hyst_last_for;
        ini_hyst_last_for = omp_get_wtime();

        #pragma omp parallel shared(pos,edge) num_threads(cant_threads)
        {
            #pragma omp for
            for(pos=0; pos<(cols*rows); pos++) {
        ...
        fin_hyst_last_for = omp_get_wtime();
        // double secs_hyst_last_for = (double)(fin_hyst_last_for - ini_hyst_last_for) / CLOCKS_PER_SEC;
        printf("Tiempo cuarto bloque paralelo: \t\t%.10f segundos\n", fin_hyst_last_for - ini_hyst_last_for);
...
unsigned char *result)
{
    ...
    ****************************************************************************/
    double ini_supress, fin_supress;
    ini_supress = omp_get_wtime();

    magrowptr=mag+ncols+1;gxrowptr=gradx+ncols+1;gyrowptr=grady+ncols+1;resultrowptr=result+ncols+1;
    // #pragma omp parallel shared(rowcount,magrowptr,gxrowptr,gyrowptr,resultrowptr,ncols) private(colcount,magptr,gxptr,gyptr,resultptr,m00,xperp,yperp,gx,gy,z1,z2,mag1,mag2) num_threads(8)
    // {
    ...
    fin_supress = omp_get_wtime();
    printf("Tiempo bloque paralelo: \t\t%.10f segundos\n", fin_supress - ini_supress);
